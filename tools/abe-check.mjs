...
//   3. Featherweight only: zero <script> tags. The page is complete as HTML.
...
const BUDGET = { gzipKB: 25, scripts: { featherweight: 0 } };
...
for (const { key, label } of THEMES_META) {
  ...
  const scriptBudget = BUDGET.scripts[key];
  const checks = [
    ...
    ...(scriptBudget !== undefined
      ? [["<script> tags = " + scriptBudget + " (zero-JS)", scriptTags === scriptBudget, String(scriptTags)]]
      : [["<script> tags (informational)", true, String(scriptTags)]]),
